...
//!
//! dig-did holds no key and signs nothing (INV-2). Instead it tells a caller EXACTLY what must be
//! signed: [`required_signatures`] runs each coin spend's puzzle to collect its `AGG_SIG_*`
...
/// Computes the exact signatures required to make a set of DID coin spends valid.
///
/// This is the bridge between dig-did's unsigned output and a caller's signer. Pass the
/// [`crate::DidSpend::coin_spends`] and the network's [`AggSigConstants`] (derived from the
...
/// # Errors
///
/// Returns [`DidError::Signer`] if a puzzle fails to evaluate or an `AGG_SIG` condition carries an
/// infinity public key.
pub fn required_signatures(
    coin_spends: &[CoinSpend],
    constants: &AggSigConstants,
) -> Result<Vec<RequiredSignature>, DidError> {
    let mut allocator = Allocator::new();
    RequiredSignature::from_coin_spends(&mut allocator, coin_spends, constants)
        .map_err(|error| DidError::Signer(error.to_string()))
}
